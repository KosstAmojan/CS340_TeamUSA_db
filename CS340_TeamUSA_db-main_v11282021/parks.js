module.exports = function(){
  ...
  function getParks(res, mysql, context, complete){
      mysql.pool.query("SELECT parkID, name, maxOccupancy, dateBuilt FROM Parks", function(error, results, fields){
          if(error){
  ...
  function getParksWithNameLike(req, res, mysql, context, complete) {
     var query = "SELECT parkID, name, maxOccupancy, dateBuilt FROM Parks where name LIKE " + mysql.pool.escape(req.params.s + '%');
    console.log(query)
  ...
  function getPark(req, res, mysql, context, complete){
      var sql = "SELECT parkID, name, maxOccupancy, dateBuilt FROM Parks WHERE parkID = ?";
  ...
  router.get('/search/:s', function(req, res){
      var callbackCount = 0;
      var context = {};
      context.jsscripts = ["deletepark.js","filterpark.js","searchpark.js","updatepark","change_page.js"];
      var mysql = req.app.get('mysql');
